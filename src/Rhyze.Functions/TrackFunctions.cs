...
namespace Rhyze.Functions
{
    public class TrackFunctions
    {
        ...
        public async Task OnTrackUploadedAsync(
            [BlobTrigger("audio/{name}")] ICloudBlob audioBlob,
            [Blob("image/{name}", FileAccess.Write)] CloudBlobContainer imageContainer,
            ILogger log)
        {
            ...
            try
            {
                ...
                imageBlob = await UploadArtworkAsync(ownerId, audioBlob.Name, imageContainer, tag);
